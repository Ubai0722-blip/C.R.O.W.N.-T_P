...
class SceneDetector:
    ...
    def _load_configs(self):
        """从 YAML 加载语气和场景配置"""
        # 加载语气库
        def _get_bindings_from_persona():
            import yaml as _yaml
            _cfg_path = Path("config.yaml")
            try:
                with open(_cfg_path, "r", encoding="utf-8") as _f:
                    _cfg = _yaml.safe_load(_f) or {}
                persona_name = _cfg.get("default_persona", "Theresa")
                _p_path = Path(f"personas/{persona_name}.yaml")
                if _p_path.exists():
                    with open(_p_path, "r", encoding="utf-8") as _pf:
                        _p_cfg = _yaml.safe_load(_pf) or {}
                        act_tone = _p_cfg.get("tone_group", "default")
                        act_scene = _p_cfg.get("scene_group", "default")
                        return act_tone, act_scene
            except:
                pass
            return "default", "default"
            
        active_tone, active_scene = _get_bindings_from_persona()
